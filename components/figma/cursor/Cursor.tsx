import CursorSVG from '@/public/assets/CursorSVG';
import React from 'react'

type Props = {
    color: string ;
    message: string ;
    x: number ;
    y:number
}

const Cursor = ({ color, message, x, y } : Props) => {
  return (
    <div className='pointer-events-none absolute top-0 left-0 ' style={{transform : `translateX(${x}px) translateY(${y}px)`}}>
        <CursorSVG color={color}/>
        {/* the ability to see the message besides the cursor */}
        {message && (
            <div className="absolute left-2 px-4 py-2 top-5 rounded-[20px]" style={{backgroundColor: color}}>
                <p className="text-white whitespace-nowrap text-sm leading-relaxed">{message}</p>
            </div>
        )}
    </div>
  )
}

export default Cursor